...
#    cms.PSet(cut=cms.string("isEB && full5x5_r9>0.85"), ##EB high R9
    cms.PSet(cut=cms.string("isEB && full5x5_r9>=0.0"), ##EB high R9
             selection = cms.VPSet(
...
#    cms.PSet(cut=cms.string("isEE && full5x5_r9>0.90"),  ##EE high R9
    cms.PSet(cut=cms.string("isEE && full5x5_r9>=0.0"),  ##EE high R9
             selection = cms.VPSet(
            cms.PSet(max=cms.string("100000.0"), 
                     rhocorr=phoEffArea,
                     ),
...
#    cms.PSet(cut=cms.string("isEB && full5x5_r9<=0.85"),  #EB low R9
    cms.PSet(cut=cms.string("isEB && full5x5_r9>=0.0"),  #EB low R9
             selection = cms.VPSet(
            cms.PSet(max=cms.string("100000.0"), #Before: 4.0
                     rhocorr=phoEffArea,
                     ),
...
#    cms.PSet(cut=cms.string("isEE && full5x5_r9<=0.90"),  ##EE low R9
    cms.PSet(cut=cms.string("isEE && full5x5_r9>=0.0"),  ##EE low R9
             selection = cms.VPSet(
            cms.PSet(max=cms.string("100000.0"), # Before: 4.0 
                     rhocorr=phoEffArea,
                     ),
...
#       Pixel seed veto already implemented in the dumper.
#        " && (!leadingPhoton.hasPixelSeed) && (!subLeadingPhoton.hasPixelSeed)"#low mass
#        " && (leadingPhoton.pt > mass/1.875 && subLeadingPhoton.pt > mass/3.)"#low mass run II ?
#        " && (leadingPhoton.pt > mass/3. && subLeadingPhoton.pt > mass/4.)#low mass run I
#        " && mass > 55 "#low mass
